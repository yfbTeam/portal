...
namespace PMSDAL
{
    public class DYYX_BaseDal<T> where T : class , new()
    {
        #region 增删改查的辅助方法
        /// <summary>
        /// 添加数据的辅助方法
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public virtual T GetEntityById(T entity, int id, string sqlWhere)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(SQLHelp.conStr))
        ...
        /// <param name="ids"></param>
        /// <returns></returns>
        public virtual int BatchChangeValue(T entity,string field,string fieldValue, params string[] ids)
        {
            string sql = string.Empty;
            StringBuilder strFirst = new StringBuilder();
            List<SqlParameter> pms = new List<SqlParameter>();
            ...
            sql = string.Format("update {0} set {1}=@FieldValue where id in({2})", entity.GetType().Name, field, strFirst.ToString().TrimEnd(','));
